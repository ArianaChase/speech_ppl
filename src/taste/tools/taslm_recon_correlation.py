...
def load_and_merge_data(csv_path, json_path):
    ...
    # Force 'Audio filename' to be read as string to preserve leading zeros
    ...
    # Map the age and gender based on 'Audio filename' mapping to JSON keys
    ...
    # Ensure age is numeric for >= and < comparisons
...
def calc_correlation(df, metric_col, human_col):
    ...
    # Return None if we don't have enough data points to compute correlation
    if len(x) < 2:
        return None
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate Demographics Correlations")
    
    # New arguments required for gspread tracking
    parser.add_argument("--name", type=str, default="Taslm_Demographics")
    parser.add_argument("--index", type=int, default=1)
    parser.add_argument("--model", type=str, default="TASLM")
    parser.add_argument("--category", type=str, default="Demographics")
    
    # Data arguments
    parser.add_argument("--csv_file", type=str, required=True, help="Path to taslm reconstruction CSV")
    ...
    metrics = ["Raw MSE", "Raw MCD"]
    
    # Capture final execution time and duration
    now = datetime.now() 
    ...
    for cat_name, cat_df in categories.items():
        ...
        # Determine number of unique speakers and samples in this specific slice
        ...
        for metric in metrics:
            # We append the subgroup name to the metric (e.g. "Raw MSE_Males") 
            # so we know exactly what is logged in the spreadsheet
            MODEL_NAME = "TASLM"
            MODEL_ID = f"{args.name}_{cat_name}_{metric}_{str(args.index)}"
