...
def parse_delta(data):
    ...
    sub_percent = (df["clean_delta"] > 0).mean() * 100 # percentage of positive values
    dist_percent = (df["sub_delta"] < 0).mean() * 100
    total_percent = (df['total_delta'] < 0).mean() * 100 
    
    return sub_percent, dist_percent, total_percent
...
if __name__ == "__main__":
    ...
    for audio_version in ['clean', 'sub']:
        for text_version in ['clean', 'sub']:
            ...
            csv_path = f"{CSV_DIR}/{MODEL_TYPE}_{MODEL_NAME}_{audio_version}_{text_version}_per_token_losses.csv"
    ...
    sub_percent, dist_percent, total_percent = parse_delta(final_data)

    # record results
    append_to_sheet([MODEL_TYPE, MODEL_NAME, sub_percent, dist_percent, total_percent, len(final_data)], SERVICE_ACCOUNT)
