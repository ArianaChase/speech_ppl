...
def create_csv_file(output_dir, model, index, notes): # gslm_001
    filename = '%s/%s_%s_%s' % (output_dir, model, index, notes)
...
def get_directory_losses(dir, csv_name, spk, labels_dict):
    ...
    for files in pbar:
        ...
        audio_file_name= os.path.basename(filename)[:9]

        if audio_file_name in labels_dict.keys():
            print(f"Loss calc filename {audio_file_name} exists in labels_dict.")
            ...
            with open(output_csv, mode="a", newline="") as csvfile:
                fieldnames = ["Speaker", "Audio filename", "Raw Mean of Per Token Losses", "Normalized Per Token Losses"]
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writerow({"Speaker": speaker, "Audio filename": os.path.basename(filename)[:9], "Raw Mean of Per Token Losses": per_token_losses_mean.item()})
        else:
            print(f"Loss calc filename {audio_file_name} does NOT exist in labels_dict, skipping...")
            pass

def parse_accuracy_scores(filename, labels_csv):
    with open(labels_csv, mode="w", newline="") as csvfile:
        fieldnames = ["Audio filename", "Accuracy Score"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

    extreme_accuracy_scores = {}
    with open(filename) as json_data:
        data = json.load(json_data)
        for audio_file in data:
            value = data[audio_file]
            with open(labels_csv, mode="a", newline="") as csvfile:
                fieldnames = ["Audio filename", "Accuracy Score"]
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writerow({"Audio filename": audio_file, "Accuracy Score": value["accuracy"]})
            
            if value["accuracy"] <= 3 or value["accuracy"] >= 8:
                extreme_accuracy_scores[os.path.basename(audio_file)] = value["accuracy"]

    return extreme_accuracy_scores
...
if __name__ == "__main__":
    ...
    labels_csv = args.output_dir + "gslm" + "001" + "_labels"
    accuracy_scores = parse_accuracy_scores(score_labels, labels_csv)
    ...
    output_csv = create_csv_file(args.output_dir, "gslm", "001", "extremes")
    ...
    for dirs in pbar:
        ...
        if int(speaker) != 1076:
            ...
            get_directory_losses(dir_path, output_csv, speaker, accuracy_scores)
        #counter += 1
